...
Search context lives in visible service prose, FAQs, the About page and CVs, with matching structured data. `llms.txt` maps the public site and `agent-info.json` provides a static structured overview. The homepage includes a copyable independent fit-assessment prompt. Neither resource provides booking or live availability; there is no booking API or MCP server.
