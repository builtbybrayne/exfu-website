...
## Homepage AI prompt corner panel, 2026-09-10

User asked to move the inline AI prompt into a dismissible, reopenable bottom-corner popup and asked about UX. Implemented a collapsed-by-default native details launcher, labelled For your AI, with the unchanged copyable prompt. Close button, Escape with focus return, outside click and focus departure dismiss the nonmodal panel. It stays usable without JavaScript, fits narrow viewports and is hidden in print. No auto-open, motion or timer. User subsequently proposed a short initial opening, pulse and countdown; agent recommended a brief introductory label instead, with no countdown and reduced-motion support. That proposal is not implemented or recorded as accepted. UX reference: https://www.nngroup.com/articles/popups/ .

Verification: build and Astro check passed; 32 targeted Chromium/Firefox tests passed for accessible pages, responsive widths, copying, keyboard close/reopen and no-JavaScript mobile panel. Formatting and whitespace checks passed.

## Animated agent invitation approved, 2026-09-10

User approved the proposed paper-and-graphite corner animation. Added a nine-second SVG/CSS sequence: staggered paper fragments, a drawn graphite connection, gathering into a briefing, drawn tick, invitation text and a final fold into the persistent button. Uses the selected recognisable-work palette and paper language. The sequence plays once per tab session via optional sessionStorage. Hover, keyboard focus and background-tab visibility pause it. Clicking immediately opens the existing prompt; Escape dismisses; reduced motion presents a static invitation. Replay introduction in the prompt panel permits explicit replay. No countdown or pulsing. Native details fallback still works without JavaScript. Browser screenshot review checked the mobile corner placement and prompt panel.

## Actual popover assembly replaces decorative teaser, 2026-09-10

User rejected the separate, boxed animation as boring and misleading, requesting transparent UI cards/layers forming the actual popover. Replaced the decorative SVG teaser with the real heading, copyable prompt and footer as independently animated paper surfaces. No outer background. Cards enter using staggered 3D transforms, settle into a usable popover, then stack and shrink toward the actual button with a receiving motion. First visit opens the popover automatically for this sequence, superseding the previous collapsed-only introduction. Any pointer or keyboard engagement settles it and cancels automatic dismissal. Reduced motion shows a static real popover. Explicit Replay assembly remains available. Updated session key makes this revision visible once per tab session.

Build and type checks passed. Initial focused suite passed 14 checks and exposed a replay-focus bug plus marginal contrast in the existing Show all roles control. Both fixed; four targeted Chromium/Firefox assembly and reduced-motion accessibility tests then passed. Inspected the assembled layers in the browser.
...
## Selected hero and coordinated timing, 2026-09-10

User chose Exploded machine. It now plays on the ordinary homepage; comparison controls remain available only through the review query. User also required card 1 always above 2 and 2 above 3. Explicit stacking levels and a flat parent stacking context preserve this order while each card can retain its own internal depth animation; shuffle no longer overrides this requirement.

User requested smoother FAB collapse and a 2–3 second initial delay. Replaced successive bounce/restart animations with one continuous 900ms timeline per card, retaining sequential bottom-first docking and downward travel of the upper cards. Automatic opening waits 2500ms of visible-page time. Early manual opening cancels the pending introduction; hidden tabs pause the initial wait. This supersedes the earlier immediate-on-load request.

Type/build and whitespace checks passed. Ten Chromium/Firefox checks passed for delayed introduction, manual override, collapse interruption, reduced motion and comparison controls. After adopting the selected default, the production build and four additional comparison/reduced-motion checks passed. Live homepage screenshot reviewed with comparison controls hidden and fixed stacking order.

## Three FAB collapse studies, 2026-09-10

User requested all three proposed physical collapse treatments for visual selection, then asked to put Copy prompt below the prompt. Added query-activated `/?fab-review=slot` comparison (also envelope and roller) with option buttons, Replay collapse and Slow motion. Studies use the actual cards and preserve bottom-first order: slot clips paper progressively at the button mouth, envelope adds a rotating lower-half reverse surface, roller winds the sheet away behind a shaded cylinder. Each uses a receiving lip on the actual button. Normal speed is 1.2 seconds; slow motion doubles the duration. No final collapse choice has been accepted; the ordinary homepage retains its previous collapse. Copy button is now below the prompt on both ordinary and review pages.

Visual review caught and fixed a selector collision with hero review. Comparison switching cancels old overlays and animations; reopening restores unclipped card content. Type/build and whitespace checks passed. Six Chromium/Firefox checks passed for comparisons, reduced motion, switching and copy-button placement. Two keyboard-scroll checks initially focused the still-hidden delayed panel; updated the tests to await visibility/focus and both passed. Live screenshots inspected slot, envelope and roller, including slow motion.

## Slot selected, paper wiggle and prompt arrows, 2026-09-10

User selected the slot collapse and requested a wee wiggle/jiggle. Ordinary homepage now uses the slot: alternating small rotations and lateral offsets damp from 2.4 degrees/3px to zero as paper feeds into the mouth. Bottom-first order, 1.2-second total collapse, delayed opening and reduced-motion fallback remain.

User asked to remove the scroll-to-read hint and add interactive up/down glyphs beside the prompt. Added labelled arrow buttons, scrolling 80% of the visible text area per press, smooth unless reduced motion is requested. Buttons disable at their respective bounds and update on scrolling, resizing and opening. Copy remains below the prompt. Native scrolling remains available without JavaScript; inactive scripted controls stay hidden.
